...
export default async function NotFound() {
  ...
  return (
    <main className="relative isolate grid min-h-svh w-screen min-w-[280px] grid-rows-[auto_1fr_auto] overflow-hidden bg-[#efede4] text-[#173a32] [background-image:radial-gradient(circle_at_78%_44%,#f0522b14,transparent_28rem)]">
      ...
      <header className="z-20 mx-auto flex min-h-20 w-full max-w-[1440px] items-center justify-between gap-8 border-b border-[#173a32]/20 px-5 pt-[env(safe-area-inset-top)] text-[0.72rem] uppercase tracking-[0.11em] sm:px-8 lg:px-12">
        <Link
      ...
      <div className="relative mx-auto grid w-full max-w-[1440px] grid-cols-1 items-center gap-4 px-5 py-8 sm:px-8 sm:py-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(20rem,0.9fr)] lg:gap-[clamp(2rem,6vw,8rem)] lg:px-12 lg:py-[clamp(3rem,7vh,6.5rem)]">
        ...
        >
          <p className="mb-[clamp(1.2rem,3vh,2.25rem)] flex items-center gap-3 text-[0.68rem] font-bold uppercase tracking-[0.16em] text-[#f0522b]">
            <span
          ...
          <p
            className={cn(
              '-ml-[0.045em] whitespace-nowrap text-[clamp(8rem,38vw,15rem)] leading-[0.58] tracking-[-0.085em] [font-variant-numeric:lining-nums_tabular-nums] lg:text-[clamp(8.5rem,20vw,18.5rem)]',
          ...
          <div className="mt-[clamp(2.6rem,6vh,4.5rem)] grid max-w-[39rem] grid-cols-1 items-start gap-5 border-t border-[#173a32]/20 pt-5 sm:grid-cols-[minmax(0,1.15fr)_minmax(12rem,0.85fr)] sm:gap-[clamp(1.5rem,4vw,3.5rem)]">
            <h1
              id="not-found-title"
              className={cn(
                'max-w-[12ch] text-balance text-[clamp(2.4rem,4.3vw,4.5rem)] leading-[0.94] tracking-[-0.045em] sm:max-w-[11ch]',
                bodoni72OldstyleBook.className,
              )}
            >
              {t('title')}
            </h1>
            <p className="max-w-lg text-pretty text-[clamp(0.88rem,1.1vw,1rem)] leading-[1.72] text-[#5f706b]">
              {t('description')}
            </p>
          </div>

          <nav
            className="mt-[clamp(2rem,4vh,3rem)] flex flex-wrap items-stretch gap-4 sm:items-center sm:gap-6"
        ...
        <div
          className="pointer-events-none absolute right-[-13rem] top-1/2 -z-10 aspect-square w-[29rem] -translate-y-1/2 opacity-25 duration-1000 animate-in fade-in slide-in-from-bottom-6 fill-mode-both motion-reduce:animate-none lg:relative lg:right-auto lg:top-auto lg:z-0 lg:w-[min(38vw,31rem)] lg:translate-y-0 lg:justify-self-center lg:opacity-100"
          aria-hidden="true"
        >
          ...
          <div className="absolute inset-[5%] rounded-full border border-[#173a32]/20">
            ...
            <div className="absolute inset-[31%] rounded-full border border-[#173a32]/20 bg-[#efede4]/40" />
          </div>
          <div className="absolute inset-[21%] rotate-[-18deg] scale-y-[0.38] rounded-full border border-[#f0522b]/55" />
      ...
      <footer className="z-20 mx-auto flex min-h-16 w-full max-w-[1440px] items-center justify-start gap-8 border-t border-[#173a32]/20 px-5 pb-[env(safe-area-inset-bottom)] text-[0.58rem] font-semibold uppercase tracking-[0.13em] text-[#5f706b] sm:justify-between sm:px-8 lg:px-12">
        <p>{t('footer')}</p>
